...
export default function PageMemberItemChild(props:DefaultComponentProps & {
  ...
  return <li className="border-b last:border-none border-stone-200/100 py-3">
    ...
    <div className="text-sm flex mt-3">
      <span className="flex-shrink-0 w-2/12 flex items-center">授权访问：<input disabled={updateMutating} type="checkbox" defaultChecked={data.authorized} onClick={handleClickAuthorized} /></span>
      <span className="flex-shrink-0 w-2/12 flex items-center">允许桥接：<input disabled={updateMutating} type="checkbox" defaultChecked={data.activeBridge} onClick={handleClickActiveBridge} /></span>
      <span className="flex-shrink-0 w-2/12 flex items-center">不自动分配托管地址：<input disabled={updateMutating} type="checkbox" defaultChecked={data.noAutoAssignIps} onClick={handleClickNoAutoAssignIps} /></span>
