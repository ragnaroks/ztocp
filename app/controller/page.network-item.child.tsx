...
export default function PageNetworkItemChild(props:DefaultComponentProps & {
  ...
  return <li className={className+' flex'}>
    <fieldset className="flex-shrink-0 mr-8">网络：{props.networkId}</fieldset>
    ...
    <fieldset className="flex-shrink-0 flex text-xs">
      <button disabled={deleteMutating} className="w-14 border border-site-zerotier/100 text-orange-500/100 mr-8" onClick={handleDeleteNetwork}>删除</button>
      <button disabled={deleteMutating} className="w-14 bg-site-zerotier/100" onClick={handleSelectNetwork}>设置</button>
    </fieldset>
  </li>;
};
